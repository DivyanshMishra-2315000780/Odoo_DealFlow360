...
export const mockStore = {
  ...
  updateInvoiceStatus(id: string, status: InvoiceStatus, paymentMethod?: string): Invoice | null {
    ...
    const updatedInvoice: Invoice = {
      ...invoices[idx],
      status,
      paymentStatus: status === 'PAID' ? 'PAID' : status === 'PARTIALLY_PAID' ? 'PARTIALLY_PAID' : 'ISSUED',
  ...
  recordInvoicePayment(
    ...
    const updatedInvoice: Invoice = {
      ...
      remainingAmount,
      status: isFullyPaid ? 'PAID' : isPartiallyPaid ? 'PARTIALLY_PAID' : 'ISSUED',
      paymentStatus: isFullyPaid ? 'PAID' : isPartiallyPaid ? 'PARTIALLY_PAID' : 'ISSUED',
      paymentMethod,
      paymentReference,
