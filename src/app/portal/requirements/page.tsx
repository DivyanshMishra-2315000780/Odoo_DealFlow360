...
export default function CustomerRequirementsPage() {
  ...
  const { data: allRequirements = [], isLoading } = useRequirements();

  // Filter strictly for this authenticated customer context
  const myRequirements = allRequirements.filter((r) => {
    if (!customerName) return true;
    const searchName = customerName.toLowerCase();
    const reqCust = r.customerName.toLowerCase();
    return reqCust.includes(searchName) || searchName.includes(reqCust);
  });
