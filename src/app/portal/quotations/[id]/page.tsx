...
export default function CustomerQuotationDetailPage(){
 const {id}=useParams<{id:string}>();const {user,isLoading:isAuthLoading}=useAuth();const customerReady=!isAuthLoading&&user?.role==='CUSTOMER'&&Boolean(user.customerId);const query=useQuotation(id,customerReady);const mutation=useUpdateQuotationStatus();const {toast}=useToast();
 const [changes,setChanges]=useState<Record<string,{quantity?:number;discountPercent?:number}>>({});const [note,setNote]=useState('');
 if(isAuthLoading||query.isLoading)return <p>Loading quotation...</p>;
