...
export default function CustomerQuotationsPage() {
  ...
  const { data: quotations = [], isLoading } = useQuotations(ready);
            ...
            <TableBody>
              {myQuotations.map((q) => (
                <TableRow key={q.id} className="hover:bg-slate-50/80 transition">
                  <TableCell>
                      <span className="font-semibold text-slate-900 text-xs block">{q.title}</span>
                      <span className="font-mono text-[10px] text-slate-400 block">Ref: {q.id}</span>
