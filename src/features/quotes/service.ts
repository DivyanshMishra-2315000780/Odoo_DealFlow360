...
} from "@/features/quote-requests/repository";
import {
  closeNegotiation,
  createApprovalChain,
  createNegotiation,
  ...
  findDefaultPriceList,
  findSelectedPriceList,
  findOpenNegotiation,
  findPricedProducts,
  findProductsByIds,
  findQuote,
  ...
  listActiveApprovalRules,
  listActiveDiscountRules,
  listInventoryForProducts,
  listQuoteLines,
  listQuotesFor,
  listRecommendations,
  listUpsellRulesFor,
...
async function assertVisible(
  user: Awaited<ReturnType<typeof requireAuth>>,
  quote: { customerId: string; salesExecId: string },
) {
  if (user.role === "CUSTOMER") {
    let custId = user.customerId;
    if (!custId) {
      const cust = await findCustomerByUserId(user.userId);
      custId = cust?.id ?? null;
    }
    if (!custId || quote.customerId !== custId) throw new AuthorizationError();
  }
  if (user.role === "SALES_EXECUTIVE" && quote.salesExecId !== user.userId)
...
export async function listQuotes() {
  ...
  let custId = user.customerId;
  if (user.role === "CUSTOMER" && !custId) {
    const cust = await findCustomerByUserId(user.userId);
    custId = cust?.id ?? null;
  }
  const rows = await listQuotesFor(
    user.role === "CUSTOMER" ? (custId ?? "") : user.userId,
    user.role,
  );
...
export async function getQuote(id: string) {
  ...
  await assertVisible(user, quote);
  ...
  if (
    ...
    !quote.reapprovalRequired
  ) {
    throw new AuthorizationError(
      "Quotation is not yet shared with the customer",
    );
  }
  const {
    totalCost,
  ...
  return {
    ...
    negotiation: negotiation
      ? {
          customerNotes: negotiation.customerNotes,
...
export async function createQuote(input: unknown) {
  ...
  if (user.role !== "SALES_EXECUTIVE")
    throw new AuthorizationError("A sales executive must create the quotation");
  const parsed = createQuoteInput.parse(input);
  const quoteRequest = parsed.quoteRequestId
    ? await findQuoteRequest(parsed.quoteRequestId)
    : null;
  if (parsed.quoteRequestId && !quoteRequest)
    throw new BusinessError("Quote request not found", "NOT_FOUND", 404);
  if (quoteRequest && quoteRequest.status !== "ASSIGNED")
    throw new BusinessError(
      "Quote request must be assigned before quotation creation",
      "INVALID_STATE",
      409,
    );
  if (
    ...
    quoteRequest.assignedSalesExecId !== user.userId
  )
    throw new AuthorizationError();
  const customerId = quoteRequest?.customerId ?? parsed.customerId;
  if (!customerId)
    throw new BusinessError("Customer is required", "VALIDATION_ERROR", 400);
  const defaultPriceList = parsed.priceListId
    ? await findSelectedPriceList(parsed.priceListId)
    : await findDefaultPriceList();
  const priceListId = parsed.priceListId ?? defaultPriceList?.id;
  if (!priceListId || !defaultPriceList)
    throw new BusinessError(
      "An active price list is required",
      "PRICE_LIST_REQUIRED",
      400,
    );
  ...
  const pricedProducts = await findPricedProducts(
    priceListId,
    parsed.lines.map((line) => line.productId),
  );
  const priceByProduct = new Map(
    pricedProducts.map((record) => [record.product.id, record]),
  );
  if (
    priceByProduct.size !==
    new Set(parsed.lines.map((line) => line.productId)).size
  ) {
    throw new BusinessError(
      "One or more products are not available on the selected price list",
      "PRICE_NOT_FOUND",
      400,
    );
  }
  ...
  const calculatedLines = parsed.lines.map((input) => {
    ...
    if ((priced.product.metadata.status ?? "ACTIVE") !== "ACTIVE")
      throw new BusinessError(
        "Product is not active for new quotations",
        "PRODUCT_UNAVAILABLE",
        400,
      );
    if (priced.product.isRecurring && !priced.product.subscriptionPlanId) {
      throw new BusinessError(
        `Recurring product ${priced.product.name} has no subscription plan`,
    ...
    const amounts = calculateLineAmounts(
      input.quantity,
      priced.price.unitPrice,
      priced.product.baseCost,
    ...
    return {
      ...
      unitCost: priced.product.baseCost,
      ...
      lineCost: amounts.lineCost.toString(),
      lineProfit: amounts.lineProfit.toString(),
      lineMarginPercentage: amounts.lineMarginPercentage.toString(),
      isRecurring: priced.product.isRecurring,
      subscriptionPlanId: priced.product.subscriptionPlanId,
      categoryId: priced.product.categoryId,
    };
  });
  const discountResult = evaluateQuotationDiscounts(
    { salesExecRole: user.role },
    calculatedLines,
    await listActiveDiscountRules(),
    customer.tier,
  );
  const discountByLine = new Map(
    discountResult.lineResults.map((result) => [result.lineId, result]),
  );
  const storedLines = calculatedLines.map(({ categoryId, ...line }) => {
    ...
    return {
      ...line,
      discountStatus: discount?.status as "COMPLIANT" | "EXCEEDED",
  ...
  const quote = await insertQuote({
    id,
    quoteNumber: `QT-${date.toISOString().slice(0, 10).replaceAll("-", "")}-${id.slice(0, 8).toUpperCase()}`,
    title: parsed.title,
    quoteRequestId: quoteRequest?.id,
    customerId,
    salesExecId: user.userId,
    priceListId,
    currency: defaultPriceList.currency,
    validityDate:
      parsed.validityDate ?? new Date(date.getTime() + 30 * 86400000),
    paymentTerms: parsed.paymentTerms,
    subtotal: totals.subtotal.toString(),
    totalDiscount: totals.totalDiscount.toString(),
    totalAmount: totals.netSubtotal.toString(),
    totalCost: totals.totalCost.toString(),
    totalProfit: totals.totalProfit.toString(),
    marginPercentage: totals.marginPercentage.toString(),
  ...
  const upsellRules = await listUpsellRulesFor(
    storedLines.map((line) => line.productId),
  );
  const targetIds = [
    ...new Set(upsellRules.map((rule) => rule.targetProductId)),
  ];
  const [targetProducts, targetInventory] = targetIds.length
    ? await Promise.all([
        findProductsByIds(targetIds),
        listInventoryForProducts(targetIds),
      ])
    : [[], []];
        ...
        ? [
            {
              ...
              type: rule.type as "UPSELL" | "CROSS_SELL",
  ...
  await insertRecommendations(
    recommendations.map((recommendation) => ({
      ...
      marginDelta: recommendation.marginDelta.toString(),
      revenueDelta: recommendation.revenueDelta.toString(),
    })),
  );
...
export async function editQuote(id: string, input: unknown) {
  ...
  const current = mustExist(await findQuoteDetails(id));
  await assertVisible(user, current);
  if (!["DRAFT", "REVISION_REQUIRED", "REJECTED"].includes(current.status))
    throw new BusinessError(
      "Only a draft or returned quote may be revised",
      "INVALID_STATE",
      409,
    );
  ...
  if (values.lines) {
    if (
      new Set(values.lines.map((l) => l.productId)).size !== values.lines.length
    )
      throw new BusinessError(
    ...
    if (
      current.lines.some(
        (l) => !values.lines!.some((v) => v.productId === l.productId),
      )
    )
      throw new BusinessError(
        "Existing lines must be retained for the negotiation audit trail",
        "INVALID_LINES",
        400,
      );
    const priced = await findPricedProducts(
      current.priceListId,
      values.lines.map((l) => l.productId),
    );
    const calculated = values.lines.map((input) => {
      const match = priced.find((p) => p.product.id === input.productId);
      if (!match)
        throw new BusinessError(
          "Product is not on this price list",
          "PRICE_NOT_FOUND",
          400,
        );
      ...
      const amounts = calculateLineAmounts(
        input.quantity,
        match.price.unitPrice,
        match.product.baseCost,
        input.discountPercentage,
      );
      return {
        id: old?.id ?? uuid(),
        ...
        productId: input.productId,
        quantity: input.quantity,
        unitPrice: match.price.unitPrice,
        unitCost: match.product.baseCost,
        discountPercentage: String(input.discountPercentage),
        discountAmount: amounts.discountAmount.toString(),
        grossAmount: amounts.grossAmount.toString(),
        netAmount: amounts.netAmount.toString(),
        ...
        lineProfit: amounts.lineProfit.toString(),
        lineMarginPercentage: amounts.lineMarginPercentage.toString(),
        isRecurring: match.product.isRecurring,
        subscriptionPlanId: match.product.subscriptionPlanId,
        categoryId: match.product.categoryId,
      };
    });
    const discounts = evaluateQuotationDiscounts(
      { salesExecRole: "SALES_EXECUTIVE" },
      calculated,
      await listActiveDiscountRules(),
      current.customer!.tier,
    );
    for (const { categoryId, ...line } of calculated) {
      void categoryId;
      const result = discounts.lineResults.find((r) => r.lineId === line.id)!;
      const stored = {
        ...line,
        discountStatus: result.status as "COMPLIANT" | "EXCEEDED",
        excessDiscountPct: result.excessDiscount.toString(),
      };
      if (current.lines.some((l) => l.id === line.id))
        await updateQuoteLine(line.id, stored);
      else await insertQuoteLines([stored]);
    }
    const totals = calculateQuoteTotals(calculated);
    const risk = calculateRisk(
      { customerTier: current.customer!.tier },
      discounts,
      totals.marginPercentage,
      totals.netSubtotal,
    );
    await updateQuote(id, {
      ...
      marginPercentage: totals.marginPercentage.toString(),
      riskScore: Math.round(risk.riskScore),
      riskLevel: risk.riskLevel as typeof current.riskLevel,
      riskReasons: risk.riskReasons,
    });
  }
  await updateQuote(id, {
  ...
  await audit(user, id, "REVISE_QUOTE", current, {
    version: current.version + 1,
  });
  return findQuoteDetails(id);
}
...
async function transitionQuote(
  ...
  if (!allowed.includes(current.status))
    throw new BusinessError(
      `Cannot transition ${current.status} to ${target}`,
      "INVALID_STATE",
      409,
    );
...
export const sendQuote = (id: string) =>
  transitionQuote(id, ["APPROVED"], "SENT", "SEND_QUOTE", "QUOTE_SEND");
export async function confirmQuote(id: string) {
  ...
  if (!["SENT", "APPROVED"].includes(quote.status))
    throw new BusinessError(
      "Only an approved or sent quote can be accepted",
      "INVALID_STATE",
      409,
    );
  if (quote.validityDate < new Date())
    throw new BusinessError("Quotation has expired", "QUOTE_EXPIRED", 409);
  ...
  await audit(
    ...
    "ACCEPT_QUOTE",
    { status: quote.status },
    { status: "FULFILLMENT", orderId: result.order.id },
  );
  return result;
}
...
export async function submitQuote(id: string) {
  ...
  await assertVisible(user, quote);
  if (
    !["DRAFT", "REVISION_REQUIRED", "REJECTED", "UNDER_NEGOTIATION"].includes(
      quote.status,
    )
  ) {
    throw new BusinessError(
      "Only draft, revised, or negotiated quotes can be submitted",
      "INVALID_STATE",
      409,
    );
  }
  if (quote.status === "UNDER_NEGOTIATION") {
    ...
    if (negotiation) {
      const res = await reviewNegotiation(id, {
        action: "APPROVE",
        comment: "Submitted for re-approval",
      });
      return res.quote;
    }
  }
...
export async function startNegotiation(id: string, input: unknown) {
  ...
  if (!["SENT", "APPROVED"].includes(quote.status))
    throw new BusinessError(
      "Only sent or approved quotes can be negotiated",
      "INVALID_STATE",
      409,
    );
  ...
  if (existing)
    throw new BusinessError(
      "An active negotiation already exists",
      "NEGOTIATION_EXISTS",
      409,
    );
  ...
  const changes = values.changes.map((change) => {
    ...
    if (!line)
      throw new BusinessError("Quotation line not found", "NOT_FOUND", 404);
    if (
      change.fieldChanged === "quantity" &&
      (!Number.isInteger(change.requestedValue) || change.requestedValue <= 0)
    ) {
      throw new BusinessError(
    ...
    if (
      change.fieldChanged === "discountPercentage" &&
      change.requestedValue > 100
    )
      throw new BusinessError(
    ...
    if (change.requestedValue < 0)
      throw new BusinessError(
        "Requested value cannot be negative",
        "INVALID_CHANGE",
        400,
      );
    return {
      ...
      originalValue: String(line[change.fieldChanged]),
...
export async function reviewNegotiation(id: string, input: unknown) {
  ...
  if (quote.status !== "UNDER_NEGOTIATION")
    throw new BusinessError(
      "Quotation is not under negotiation",
      "INVALID_STATE",
      409,
    );
  const negotiation = await findOpenNegotiation(id);
  if (!negotiation)
    throw new BusinessError("Active negotiation not found", "NOT_FOUND", 404);
  ...
  await saveQuoteVersion(
    ...
    quote.version,
    quote,
    values.comment ?? "Negotiation accepted",
    user.userId,
  );
  ...
  for (const change of negotiation.changes) {
    ...
    const quantity =
      change.fieldChanged === "quantity"
        ? Number(change.requestedValue)
        : line.quantity;
    const unitPrice =
      change.fieldChanged === "unitPrice"
  ...
  const productRows = await findProductsByIds(
    [...lineMap.values()].map((line) => line.productId),
  );
  const discounts = evaluateQuotationDiscounts(
    { salesExecRole: "SALES_EXECUTIVE" },
    [...lineMap.values()].map((line) => ({
      ...line,
      categoryId: productRows.find((p) => p.id === line.productId)?.categoryId,
    })),
    await listActiveDiscountRules(),
    quote.customer!.tier,
  );
  for (const result of discounts.lineResults)
    await updateQuoteLine(result.lineId!, {
      discountStatus: result.status as "COMPLIANT" | "EXCEEDED",
      excessDiscountPct: result.excessDiscount.toString(),
    });
  const risk = calculateRisk(
  ...
  const reevaluated = mustExist(
    await updateQuote(id, {
      ...
      marginPercentage: totals.marginPercentage.toString(),
      riskScore: Math.round(risk.riskScore),
      riskLevel: risk.riskLevel as typeof quote.riskLevel,
      riskReasons: risk.riskReasons,
    }),
  );
  const chain = determineApprovalChain(
  ...
  await audit(user, id, "APPROVE_NEGOTIATION", quote, {
    ...reevaluated,
    status: target,
  });
  return {
    quote: { ...reevaluated, status: target },
    approvalRequired: chain.required,
  };
}
