...
export async function findQuoteRequest(id: string) {
  ...
  const [quote] = await db.select({ id: quotations.id, title: quoteRequests.title })
    .from(quotations)
    .innerJoin(quoteRequests, eq(quotations.quoteRequestId, quoteRequests.id))
    .where(eq(quotations.quoteRequestId, id))
    .limit(1);
  return { ...request, items, customer, quotationId: quote?.id, quotationTitle: quote?.title };
}
