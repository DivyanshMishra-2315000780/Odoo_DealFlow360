...
const permissions: Record<AdminResource, Permission> = {
  ...
  priceLists: Permission.MANAGE_PRICING,
  discountRules: Permission.MANAGE_PRICING,
  approvalRules: Permission.MANAGE_APPROVAL_RULES,
  warehouses: Permission.MANAGE_WAREHOUSES,
  categories: Permission.MANAGE_PRODUCTS,
  subscriptionPlans: Permission.MANAGE_SUBSCRIPTIONS,
};
...
export async function createResource(resource: AdminResource, input: unknown) {
  ...
  if (resource === "users" && record.role === "CUSTOMER") {
    await insertCustomer({
      ...
      name: String(companyName ?? `${record.firstName} ${record.lastName}`),
      contactEmail: String(record.email),
      tier: "BRONZE",
    });
  }
...
export async function removeResource(resource: AdminResource, id: string) {
  await authorize(resource);
  if (resource === "users")
    return updateResource(resource, id, { active: false });
  if (resource === "products") {
    const old = await findAdminRecord(resource, id);
    if (!old) throw new BusinessError("Resource not found", "NOT_FOUND", 404);
    return updateAdminRecord(resource, id, {
      metadata: {
